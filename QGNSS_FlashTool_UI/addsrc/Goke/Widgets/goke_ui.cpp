...
Goke_UI::Goke_UI(QWidget *parent)
    :Goke_base_window(parent)
{
  cfg_parameter();
  cfg_construct();
  cfg_widget();
  GokeModel=new Goke_Model;
  cfg_model (GokeModel);
}
...
void Goke_UI::start()
{
  ...
  GokeModel->qGNSS_SetDownLoadBaudRate (baudrate);
  statuswgt_->set_widget ({Status_WGT::Downloadding,qgnss_help_->currentmodel ()->key ()+":Downloading"});
  if(GokeModel->start_download(loaclitem->text (),qgnss_help_->currentmodel ()->value ()))
  {
...
void Goke_UI::ini_IO(IODvice *io)
{
  base->init_IO (io);
}
void Goke_UI::cfg_widget()
{
  ...
  //[0]menubar
  connect (this,&Goke_UI::currentBaudrateChanged,this,[&](int index){
  ...
  connect (this,&Goke_UI::currentDownmodeChanged,this,[&](int index){
...
//  label_DA_name_->setText (tr("BE File:"));
...
void Goke_UI::open_file(bool DAbt)
{
  QString fileName;
//  if(DAbt)
//  {
//    fileName = QFileDialog::getOpenFileName(this, QObject::tr("Select File"), nullptr, QObject::tr("BE File (*.bin)"));
...
void Goke_UI::load_file(bool DAbt)
{
  QString fileName;
//  if(DAbt)
//  {
...
bool Goke_UI::event(QEvent *event)
{
  switch (event->type ()) {
    case QEvent::Close:
      if(!statuswgt_->bt_open ())
      ...
      break;
    default:
      return Goke_base_window::event (event);
  }
  return Goke_base_window::event (event);
}

void Goke_UI::wgt_enable(bool Run)
{
  if(Run)
  {
