                         ...
                         const int *ex, const int *half)
{
    ...
    for (i=j=0;i<h->nsat;i++) {
        ...
        for (k=0;k<h->nsig;k++) {
            ...
            if (sat&&index>=0) {
                rtcm->obs.data[index].SNR [k]=(uint16_t)(cnr[j]/SNR_UNIT+0.5);
                rtcm->obs.data[index].code[k]=h->sigs[k];
            }
            j++;
        }
    }
...
extern int decode_rtcm3(rtcm_t *rtcm)
{
    ...
    /* jiejie add */
    SaveInfoToQueue(rtcm->buff,rtcm->len + 3);
    /* jiejie add --end */
    ...
    switch (type) {
        ...
        /* jiejie add */
        case  999: decode_type999(rtcm);          break;
        /* jiejie add --end */
    }
    if (ret>=0) {
