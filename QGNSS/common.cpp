...
void qgnss_sleep(int delay_time)
{
    ...
    timer.start();
    /* wait until time out */
    while (timer.elapsed() < delay_time)
    {
...
int addNMEA_CS(QByteArray &NMEA_Ste)
{
    ...
    NMEA_Ste.append(str.sprintf("*%02X\r\n",sum));
    return sum;
}
...
int add_CS(QByteArray &data, int start_p)
{
    char sum = 0;
    QString str;
    for(int i = start_p;i<data.length();i++){
        sum^=data.at(i);
    }
    data.append(sum);
    return sum;
...
unsigned int c_crc32(const unsigned char *buffer, unsigned int size, unsigned int crc)
{
    unsigned int i;
    for (i = 0; i < size; i++) {
...
int addAllystarBinaryCS(QByteArray &msg)
{
    ...
    foreach (auto var, msg.mid(2)) {
        checksum1 += var;
        checksum2 += checksum1;
    }
    msg.append(checksum1);
